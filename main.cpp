...
int main(int argc, char** argv) {
    ...
    const int VA = 0;   //4xxxy
    const int DY = 1;   //6011y
    const int MC1 = 2;  //51xxy
    const int MC2 = 3;  //52xxy
    const int MC3 = 4;  //53xxy
    const int MC4 = 5;  //54xxy
    const int MC5 = 6;  //55xxy
    const int AX1 = 7;  //34xxy
    const int AX2 = 8;  //37xxy
    const int PH = 9;
    ofstream output;
    int input = 0;
    int** cards;
...
void progressUpdater(int part)
{
    if(part == 0)
    {
        cout << "|" << setw(30) << "Generating Credit Card Numbers" << setw(35) << "[                               ]|\r";
        fflush(stdout);
        return;
    }
    if(part ==1)
    {
        cout << "|" << setw(30) << "Generating Credit Card Numbers" << setw(35) << "[ooooooooo                      ]|\r";
        fflush(stdout);
        return;
    }
    if(part ==2)
    {
        cout << "|" << setw(30) << "Checking the Validity" << setw(35) << "[oooooooooooooooooooooooo       ]|\r";
        fflush(stdout);
        return;
    }
    if(part ==3)
    {
        cout << "|" << setw(30) << "DONE" << setw(35) << "[ooooooooooooooooooooooooooooooo]|\r";
        fflush(stdout);
        cout << endl;
        return;   
    }
...
//performs the validation on all of the credit card numbers
